 ...
 */
public class BYModuleBus {
    private static final String TAG = "BYModuleBus";
...
//    private static ArrayMap<Object,ArrayMap<String,Method>> moduleEventMethods = new ArrayMap<>();
     ...
     * Object methodClass
     * String methodName；
     * BYMethodInfo method info
     */
    private static ArrayMap<Object, ArrayMap<String, BYMethodInfo>> moduleEventMethods = new ArrayMap<>();
//    private static ArrayMap<Class<?>,ArrayList<Object>> moduleClients = new ArrayMap<>();
    ...
    private static BYModuleBus instance;

    private String PK_NAME ="";

    public static BYModuleBus getInstance(){
        if(instance == null){
            synchronized (BYModuleBus.class){
                if (instance == null){
                    instance = new BYModuleBus();
                }
            }
        }
        return instance;
...
//    }
    ...
    private void addEventMethod(Object client, Method m, boolean single){
            ArrayMap<String, BYMethodInfo> methods = moduleEventMethods.get(client);
            if (methods == null) {
                methods = new ArrayMap<>();
                moduleEventMethods.put(client, methods);
            }
            methods.put(m.getName(), new BYMethodInfo(m.getName(), m, single));
    }
...
//    }
    ...
    public void post(Class<?> clientClass, String methodName, Object...args){
        ...
        try{
            for(Object c: clientList){
                try{
                    ArrayMap<String, BYMethodInfo> methods = moduleEventMethods.get(c);
    ...
    public Object postSingle(Class<?> clientClass, String methodName, Object...args){
        ...
        try{
            BYMethodInfo BYMethodInfo = moduleEventMethods.get(clientClass).get(methodName);
            boolean single = BYMethodInfo.single;
            Method method = BYMethodInfo.m;
    ...
    public static void init(Context context){
        BYModuleCenter.init(context);
    }

    public List<String> getModuleList(String templet) {
        return BYModuleCenter.getModuleList(templet);
    }
