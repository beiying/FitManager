...
import com.beiying.fitmanager.core.charts.util.FillFormatter;

public class BYLineChart extends BYBarLineChartBase<BYLineData> {
	...
	public BYLineChart(Context context) {
		super(context);
	}
	...
	protected void drawValues() {
		...
		if (mDrawYValues
			...
			ArrayList<BYLineDataSet> dataSets = mData.getDataSets();

			for (int i = 0; i < mData.getDataSetCount(); i++) {

				BYLineDataSet dataSet = dataSets.get(i);
				...
				ArrayList<BYEntry> entries = dataSet.getYVals();
	...
	protected void drawData() {
		ArrayList<BYLineDataSet> dataSets = mData.getDataSets();
		for (int i = 0; i < mData.getDataSetCount(); i++) {
			BYLineDataSet dataSet = dataSets.get(i);// 取出一条折线上的对应的值
			ArrayList<BYEntry> entries = dataSet.getYVals();
	...
	protected void drawCublic(BYLineDataSet dataSet, ArrayList<BYEntry> entries) {
		mRenderPaint.setColor(dataSet.getColor());
		float intensity = dataSet.getCubicIntensity();

		Path spline = new Path();
		ArrayList<CPoint> points = new ArrayList<BYLineChart.CPoint>();
		for (BYEntry entry : entries) {
			points.add(new CPoint(entry.getXIndex(), entry.getVal()));
		}
	...
	protected void drawCubicFill(BYLineDataSet dataSet,
			ArrayList<BYEntry> entries, Path spline) {
	...
	protected void drawLinear(BYLineDataSet dataSet, ArrayList<BYEntry> entries) {
		mRenderPaint.setStyle(Paint.Style.STROKE);
	...
	protected void drawLinearFill(BYLineDataSet dataSet,
			ArrayList<BYEntry> entries) {
	 ...
	 * @return
	 */
	private Path generateFilledPath(ArrayList<BYEntry> entries, float fillMin) {
	 ...
	 * @return
	 */
	private Path generateLinePath(ArrayList<BYEntry> entries) {
	...
	protected void drawHighlights() {
		for (int i = 0; i < mIndicesToHightlight.length; i++) {
			...
			BYLineDataSet set = mData.getDataSetByIndex(mIndicesToHightlight[i]
					.getDataSetIndex());
	...
	private class DefaultFillFormatter implements FillFormatter {
		...
		@Override
		public float getFillLinePosition(BYLineDataSet dataSet,
				BYLineData data, float chartMaxY, float chartMinY) {
